				...
				<div class="row">
					...
					<div class="col-md-5">
						<div class="form-group">
							...
							<label class="col-md-1">:</label>
							<div class="col-md-7">
								<input type="date" class="form-control" required v-model="transaction.Tr_date" @change="getTransactions" <?php echo $this->session->userdata('accountType') == 'u' ? 'disabled': '' ?>>
							</div>
						</div>
						<div class="form-group">
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			addTransaction(){
				let userType = "<?php echo $this->session->userdata('accountType') ?>"
				if(userType == 'u'){
					this.transaction.Tr_date = moment().format('YYYY-MM-DD');
				}
